		...
		if (index != -1) {
			...
			for (;;) {
				...
				if (index < 0 || index >= buddy->index_max) {
					...
					printf
					    ("      <BLOCK %s A=%x-%x ORDER=%d ME=%d NEXT=%d PREV=%d>\n",
					     IS_FREE(block) ? "free" :
					     "in use",
					     start, end,
					     GET_ORDER(block),
					     index,
					     GET_NEXT(block), GET_PREV(block)
					    );
...
static buddy_block_t *buddy_remove_free(buddy_ctxt_t * buddy, int index,
					int order)
{
...
BOOL buddy_init(buddy_ctxt_t * buddy, uint32_t base, uint32_t size,
		int page_bits, uint32_t first_free, uint32_t last_free)
{
	...
	if (page_bits < 5 || page_bits > 24 || size < 1)
		return FALSE;

	/* how many orders will this be? */
	buddy->order = 0;

	while (size > (1UL << buddy->order))
		buddy->order++;
	...
	for (i = 0; i < size; i++) {
		...
		if ((first_free == 0 || adr >= first_free)
		    && (last_free == 0 || adr < last_free)) {
...
BOOL buddy_alloc(buddy_ctxt_t * buddy, int order, uint32_t * adr)
{
	...
	for (i = order; i < buddy->order; i++) {
		...
		block->prev =
		    (block->prev & ~MASK_ORDER) | (order << SHIFT_ORDER);
...
BOOL buddy_alloc_at(buddy_ctxt_t * buddy, uint32_t adr, int order,
		    uint32_t * ret)
{
	...
	for (i = order; i < buddy->order; i++) {
		...
		for (;;) {
			...
			if (adr1 >= adr2 && end1 < end2) {
				...
				buddy_allocate_subset(buddy, index, i, adr1,
						      order);
	...
	if (index < buddy->index_max) {
		...
		if (!IS_FREE(block)) {
			...
			for (i = GET_ORDER(block); i < order_max; i++) {
				...
				if (index_buddy >= buddy->index_max)
					break;

				block_buddy = buddy->all + index_buddy;

				if (!IS_FREE(block_buddy)
				    || GET_ORDER(block_buddy) != i)
