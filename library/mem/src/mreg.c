...
PRIVATE BOOL memregion_attach_buddy(mreg_t * reg, memregion_t * mem)
{
	...
	if (mem->flags & (MEMREGION_FLAGS_IS_BUDDY_PARENT |
			  MEMREGION_FLAGS_IS_BUDDY_CHILD)) {
	...
	if (buddy) {
		...
		if (buddy_init
		    (buddy, mem->start, mem->end - mem->start + 1,
		     reg->page_bits, 0, 0)) {
...
/* ----[ level 1 ]------------------------------------------------------------ */
PRIVATE memregion_t *mreg_level1_split(mreg_t * reg, memregion_t * from,
				       size_t size, BOOL keep_first)
{

	/* split extra space and return it one of the two (free the other) */
	memregion_t *second =
	    memregion_alloc(reg, from->start + size, from->end);
...
PRIVATE memregion_t *mreg_level1_allocate_at(mreg_t * reg, addr_t adr,
					     int count)
{
	...
	if (target) {
		...
		if (target->start < adr) {
			...
			target =
			    mreg_level1_split(reg, target,
					      adr - target->start + 0, FALSE);
...
PRIVATE memregion_t *mreg_level2_allocate(mreg_t * reg, size_t size)
{
	...
	LIST_FOREACH_TYPED(&reg->free_buddies, memregion_t, tmp) {
		...
		if (buddy_alloc(tmp->buddy, order, &adr)) {
			...
			ret =
			    memregion_alloc(reg, adr,
					    adr - 1 +
					    (1UL << (order + reg->page_bits)));
...
PRIVATE memregion_t *mreg_level2_allocate_at(mreg_t * reg, addr_t adr,
					     size_t size)
{
	...
	if (size > 0) {
		...
		if (size >= MB) {
			...
			if (!ret) {
				...
				ret =
				    mreg_level1_allocate(reg, size >> MB_BITS);
