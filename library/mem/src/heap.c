...
// --------------------------------------------------
PRIVATE heap_marker_t *heap_marker_prev(heap_ctxt_t * heap,
					heap_marker_t * mark)
{
...
PRIVATE heap_marker_t *heap_marker_next(heap_ctxt_t * heap,
					heap_marker_t * mark)
{

	if (mark->size & HEAP_MASK_LAST)
		return 0;
...
PRIVATE void heap_marker_join(heap_ctxt_t * heap, heap_marker_t * m1,
			      heap_marker_t * m2)
{
...
PRIVATE heap_marker_t *heap_marker_split(heap_ctxt_t * heap,
					 heap_marker_t * mark, int size)
{

	heap_marker_t *tmp =
	    (heap_marker_t *) (size + sizeof(heap_marker_t) + (uint32_t) mark);
	...
	if (remain > SIZE_MIN) {	/* can split, enough space for the next */
		...
		tmp->size =
		    (remain -
		     sizeof(heap_marker_t)) | (mark->size & HEAP_MASK_FLAGS);
...
#ifdef DEBUG
		printf
		    ("Sanity check failure. Is this block is not ours??\n\t%08lx\n",
		     (uint32_t) tmp);
...
#ifdef DEBUG
		printf
		    ("Sanity check failure. Is this block already free??\n\t%08lx\n",
		     (uint32_t) tmp);
