...
void hypercall_dyn_new_pgd(addr_t * pgd_va)
{
	...
	if (l1_desc_entry & MMU_L1_TYPE_SECTION) {	/*Section page, replace it with lvl 2 pages */
		...
		for (i = table2_idx; i < end; i++, page_pa += 0x1000) {
			if (i >= l2_entry_idx && i < l2_entry_idx + 4) {	/*4 small pages RO 16KB PGD */
				...
				dmmu_l2_map_entry(table2_pa, i, page_pa,
						  ro_attrs);
			} else
	...
	memcpy((void *)((uint32_t) pgd_va + 0x2fC0),
...
void hypercall_dyn_set_pmd(addr_t * pmd, uint32_t desc)
{
...
#endif
	curr_pgd_va = (addr_t) (pmd) & 0xFFFFC000;	/*Mask with 16KB alignment to get PGD */
	...
	l1_pt_entry_for_desc =
	    (addr_t *) ((addr_t) curr_pgd_va + l1_pt_idx_for_desc +
			page_offset_idx);
	l1_desc_entry = *l1_pt_entry_for_desc;

	addr_t *linux_va =
	    MMU_L2_SMALL_ADDR(l1_entry) - phys_start + page_offset;
	...
	uint32_t temp = MMU_L2_SMALL_ADDR(desc);
	uint32_t temp_va = temp - phys_start + page_offset;
	uint32_t temp_idx = temp_va >> MMU_L1_SECTION_SHIFT;

	uint32_t *l2pt_va =
	    MMU_L1_PT_ADDR(curr_pgd_va[temp_idx]) - phys_start + page_offset;
	uint32_t l2_idx = ((uint32_t) desc << 12) >> 24;
	uint32_t l2entry_desc = l2pt_va[l2_idx];

	/*Get index of physical L2PT */
	uint32_t table2_idx = (temp - (temp & L2_BASE_MASK)) >> MMU_L1_PT_SHIFT;
	table2_idx *= 0x100;	/*256 pages per L2PT */

	if ((l2entry_desc >> 4) & 0xff == 3)
		dmmu_l2_unmap_entry(temp & L2_BASE_MASK, table2_idx);
	...
	addr_t virt_transl_for_pmd =
	    (addr_t) ((pmd - curr_pgd_va) << MMU_L1_SECTION_SHIFT);
	...
	if (desc == 0) {
		...
		dmmu_l1_pt_map(virt_transl_for_pmd, MMU_L2_SMALL_ADDR(desc),
			       attrs);
...
hypercall_dyn_set_pte(addr_t * l2pt_linux_entry_va, uint32_t linux_pte,
		      uint32_t phys_pte)
{
#ifdef DEBUG_MMU
	printf
	...
	uint32_t table2_idx =
	    (l2pt_hw_entry_pa -
	     (l2pt_hw_entry_pa & L2_BASE_MASK)) >> MMU_L1_PT_SHIFT;
	...
	if (phys_pte != 0) {
		dmmu_l2_map_entry(l2pt_hw_entry_pa & L2_BASE_MASK, entry_idx,
