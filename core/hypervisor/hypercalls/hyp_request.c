...
void request_dmmu_switch_mm(addr_t l1_base_pa_add) {
	...
	hypercall_request_t * request = &(pending_requests[curr_vm->pending_request_counter]);
...
void request_dmmu_create_L1_pt(addr_t l1_base_pa_add) {
	if (curr_vm->pending_request_counter >= MAX_PENDING_REQUESTS)
    		return;
	hypercall_request_t * pending_requests = (hypercall_request_t *)REQUESTS_BASE_VA;
	hypercall_request_t * request = &(pending_requests[curr_vm->pending_request_counter]);
...
void  request_dmmu_l1_pt_map(addr_t va, addr_t l2_base_pa_add, uint32_t attrs) {
	if (curr_vm->pending_request_counter >= MAX_PENDING_REQUESTS)
    		return;
	hypercall_request_t * pending_requests = (hypercall_request_t *)REQUESTS_BASE_VA;
	hypercall_request_t * request = &(pending_requests[curr_vm->pending_request_counter]);
...
void request_dmmu_unmap_L1_pt(addr_t l1_base_pa_add) {
	if (curr_vm->pending_request_counter >= MAX_PENDING_REQUESTS)
    		return;
	hypercall_request_t * pending_requests = (hypercall_request_t *)REQUESTS_BASE_VA;
	hypercall_request_t * request = &(pending_requests[curr_vm->pending_request_counter]);
...
void request_dmmu_unmap_L2_pt(addr_t l2_base_pa_add) {
	if (curr_vm->pending_request_counter >= MAX_PENDING_REQUESTS)
    		return;
	hypercall_request_t * pending_requests = (hypercall_request_t *)REQUESTS_BASE_VA;
	hypercall_request_t * request = &(pending_requests[curr_vm->pending_request_counter]);
...
void request_dmmu_unmap_L1_pageTable_entry(addr_t  va) {
	if (curr_vm->pending_request_counter >= MAX_PENDING_REQUESTS)
    		return;
	hypercall_request_t * pending_requests = (hypercall_request_t *)REQUESTS_BASE_VA;
	hypercall_request_t * request = &(pending_requests[curr_vm->pending_request_counter]);
...
void request_dmmu_create_L2_pt(addr_t l2_base_pa_add) {
	if (curr_vm->pending_request_counter >= MAX_PENDING_REQUESTS)
    		return;
	hypercall_request_t * pending_requests = (hypercall_request_t *)REQUESTS_BASE_VA;
	hypercall_request_t * request = &(pending_requests[curr_vm->pending_request_counter]);
...
void request_dmmu_l2_map_entry(addr_t l2_base_pa_add, uint32_t l2_idx, addr_t page_pa_add, uint32_t attrs) {
	if (curr_vm->pending_request_counter >= MAX_PENDING_REQUESTS)
    		return;
	hypercall_request_t * pending_requests = (hypercall_request_t *)REQUESTS_BASE_VA;
	hypercall_request_t * request = &(pending_requests[curr_vm->pending_request_counter]);
...
void request_dmmu_l2_unmap_entry(addr_t l2_base_pa_add, uint32_t l2_idx) {
	if (curr_vm->pending_request_counter >= MAX_PENDING_REQUESTS)
    		return;
	hypercall_request_t * pending_requests = (hypercall_request_t *)REQUESTS_BASE_VA;
	hypercall_request_t * request = &(pending_requests[curr_vm->pending_request_counter]);
...
void request_dmmu_map_L1_section(addr_t va, addr_t sec_base_add, uint32_t attrs) {
	if (curr_vm->pending_request_counter >= MAX_PENDING_REQUESTS)
    		return;
	hypercall_request_t * pending_requests = (hypercall_request_t *)REQUESTS_BASE_VA;
	hypercall_request_t * request = &(pending_requests[curr_vm->pending_request_counter]);
...
void dmmu_handler_2(uint32_t p03, uint32_t p1, uint32_t p2)
{
	...
	switch(p0) {
		case CMD_CREATE_L1_PT:
			request_dmmu_create_L1_pt(p1);
			break;
		case CMD_MAP_L1_SECTION:
			request_dmmu_map_L1_section(p1,p2,p3);
			break;
		case CMD_MAP_L1_PT:
			request_dmmu_l1_pt_map(p1, p2, p3);
			break;
		case CMD_UNMAP_L1_PT_ENTRY:
			request_dmmu_unmap_L1_pageTable_entry(p1);
			break;
		case CMD_FREE_L1:
			request_dmmu_unmap_L1_pt(p1);
			break;
		case CMD_CREATE_L2_PT:
			request_dmmu_create_L2_pt(p1);
			break;
		case CMD_MAP_L2_ENTRY:
			p3 = p03 & 0xFFFFFFF0;
			uint32_t idx = p2 >> 20;
			uint32_t attrs = p2 & 0xFFF;
			request_dmmu_l2_map_entry(p1, idx, p3, attrs);
			break;
		case CMD_UNMAP_L2_ENTRY:
			request_dmmu_l2_unmap_entry(p1, p2);
			break;
		case CMD_FREE_L2:
			request_dmmu_unmap_L2_pt(p1);
			break;
		case CMD_SWITCH_ACTIVE_L1:
			request_dmmu_switch_mm(p1);
			break;
		default:
			return;
	}
