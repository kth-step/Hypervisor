...
/*   let l1_idx = va >>> 20                 in */
/*   let l1_desc_add = l1Base_add !! (l1_idx ‰ª 2)in */
...
/*       let unmap_l1_desc = l1_desc && 0xfffffffcw in */
/*       let mem' = write_mem32(l1_desc_add, mem, unmap_l1_desc) in */
/*       if((l1_type = 0x2w) /\ (l1_sec_desc.typ = 0x01w)) */
/*       then */
/*       	  let pgrefs' = */
/* 	      (if (l1_sec_desc.ap = 0x011w) */
/*       	      then */
/* 		  let old_sec_refs = */
...
#define CREATE_L1_SEC_DESC(x, y) (L1_SEC_DESC_MASK & x) || (L1_SEC_DESC_ATTR_MASK & y) || (0x2)
...
uint32_t hypercall_map_l1_section(addr_t va, addr_t sec_base_add,
				  uint32_t attrs)
{
	uint32_t l1_base_add;
	uint32_t l1_idx;
	...
	if (!(sec_base_add >= (guest_start_pa)
	      && sec_base_add < (guest_start_pa + guest_size)))
		...
		*((uint32_t *) l1_desc_va_add) = l1_sec_desc;
		...
		for (sec_idx = 0; sec_idx < 256; sec_idx++) {
			...
			uint32_t ph_block =
			    ...
			    (sec_idx);
			if (ph_block < 3 * 256) {
				if ((ph_block_state[ph_block].refs == MAX_30BIT)
				    || (ph_block_state[ph_block].type !=
					PAGE_INFO_TYPE_DATA)) {
					sanity_check = FALSE;
				}
			} else {
