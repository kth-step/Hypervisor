...
BOOL l1Sec_checker(uint32_t l1_desc, addr_t l1_base_pa_add)
{
	...
	if ((ap == 3) && (START_PA_OF_SECTION(sec) == (START_PA_OF_SECTION(sec) & L1_SEC_DESC_MASK)))	// TODO: second seems to be superfluous
		for (sec_idx = 0; sec_idx < 256; sec_idx++) {
			...
			if ((ph_block_in_sec & L1_BASE_MASK) ==
			    (l1_base_pa_add >> 12)) {
				err_flag = 1;
				//break;
			}
...
BOOL l1Desc_validityChecker_dispatcher(unsigned int l1_type,
				       unsigned int l1_desc, addr_t pgd)
{
	if (l1_type == 0)
		return TRUE;
	if (l1_type == 1)
...
int dmmu_create_L1_pt(addr_t l1_base_pa_add)
{
	...
	for (i = 0; i < 4; i++) {
		if (!((l1_base_pa_add + (i * 4096)) >= (guest_start_pa)
		      && (l1_base_pa_add + (i * 4096)) <= guest_end_pa))
			return ERR_MMU_OUT_OF_RANGE_PA;
		uint32_t ph_block =
		    PA_TO_PH_BLOCK(PA_TO_PH_BLOCK(l1_desc_pa_add + (i * 4096)));
	...
	if (bft_entry[0]->refcnt != 0 ||
	    ...
	    bft_entry[0]->refcnt != 0 || bft_entry[0]->refcnt != 0)
		return ERR_MMU_REFERENCED_OR_PT_REGION;
	...
	for (l1_idx = 0; l1_idx < 4096; l1_idx++) {
		...
		if (l1Desc_validityChecker_dispatcher
		      ...
		      addr_t page_pa_add, uint32_t attrs)
{
	...
	if ((ap == 3)
	    && ((bft_entry_pg->refcnt == MAX_30BIT)
		|| (bft_entry_pg->type != PAGE_INFO_TYPE_DATA)))
...
int unmap_L2_pt(addr_t l2_base_pa_add)
{
	...
	for (l2_idx = 0; l2_idx < 1024; l2_idx++) {
		...
		dmmu_entry_t *bft_entry_pg =
		    get_bft_entry_by_block_idx(pg_desc->addr);
		ap = ((uint32_t) pg_desc->ap_3b) << 2 | pg_desc->ap_0_1bs;
		l2_type = l2_desc & DESC_TYPE_MASK;
...
int dmmu_handler(uint32_t p03, uint32_t p1, uint32_t p2)
{
	...
	case CMD_MAP_L2_ENTRY:
		return dmmu_l2_map_entry(p1, p2,
