...
BOOL l1Sec_checker(uint32_t l1_desc, addr_t l1_base_pa_add)
{
	...
	for (sec_idx = 0; sec_idx < 256; sec_idx++) {
		uint32_t sec_block =
		    PA_TO_PH_BLOCK(START_PA_OF_SECTION(sec)) | sec_idx;
		if ((PA_TO_PH_BLOCK(PG_ADDR_LOWER_BOUND) < sec_block)
		    && (sec_block + PAGE_SIZE <
			PA_TO_PH_BLOCK(PG_ADDR_UPPER_BOUND))) {
...
BOOL l2Pt_desc_ap(addr_t l2_base_pa_add, l2_small_t * pg_desc)
{
	...
	if ((PA_TO_PH_BLOCK(PG_ADDR_LOWER_BOUND) < pg_desc->addr)
	    && (pg_desc->addr + PAGE_SIZE <
		PA_TO_PH_BLOCK(PG_ADDR_UPPER_BOUND))) {
#ifdef DEBUG_DMMU_CACHEABILITY_CHECKERS
