...
BOOL l1PT_checker(uint32_t l1_desc)
{
	...
	if ((bft_entry_pt->type == PAGE_INFO_TYPE_L2PT)
	    && (bft_entry_pt->refcnt < (MAX_30BIT - 4096)) && !(pt->pxn)
	    )
		return TRUE;
	else
		return FALSE;
...
BOOL l1Sec_checker(uint32_t l1_desc, addr_t l1_base_pa_add)
{
	...
	if (ap == 3) {
		uint32_t guest_start_pa =
		    curr_vm->config->pa_for_pt_access_start;
		uint32_t guest_end_pa = curr_vm->config->pa_for_pt_access_end;
		if (!((START_PA_OF_SECTION(sec) >= (guest_start_pa))
		      && ((START_PA_OF_SECTION(sec) + (SECTION_SIZE - 1)) <=
			  guest_end_pa)))
			err_flag = 1;
...
BOOL l1Desc_validityChecker_dispatcher(uint32_t l1_type, uint32_t l1_desc,
				       addr_t pgd)
{
	if (l1_type == 0)
		return TRUE;
...
int dmmu_create_L1_pt(addr_t l1_base_pa_add)
{
	...
	uint32_t guest_end_pa = curr_vm->config->pa_for_pt_access_end;
	if (!(l1_base_pa_add >= (guest_start_pa)
	      && l1_base_pa_add + 4 * PAGE_SIZE < guest_end_pa))
	...
	for (l1_idx = 0; l1_idx < 4096; l1_idx++) {
		...
		if (L1_TYPE(l1_desc) != UNMAPPED_ENTRY) {
			...
			l1_desc_va_add =
			    mmu_guest_pa_to_va(l1_desc_pa_add, curr_vm->config);
			if (((l1_desc & 0xFFFF0000) == l1_base_pa_add)
			    || ((l1_desc & 0xFFFF0000) == 0x81200000))
				*((uint32_t *) l1_desc_va_add) =
				    (l1_desc & 0xFFFFFBFF);
			else
...
uint32_t dmmu_map_L1_section(addr_t va, addr_t sec_base_add, uint32_t attrs)
{
	...
	if (!(sec_base_add >= (guest_start_pa)
	      && sec_base_add + SECTION_SIZE < guest_end_pa))
		return ERR_MMU_OUT_OF_RANGE_PA;
...
BOOL l2Pt_desc_ap(addr_t l2_base_pa_add, l1_small_t * pg_desc)
{
	...
	if (ap == 3) {
		...
		uint32_t guest_start_pa =
		    curr_vm->config->pa_for_pt_access_start;
		uint32_t guest_end_pa = curr_vm->config->pa_for_pt_access_end;
		if (!((START_PA_OF_SPT(pg_desc) >= (guest_start_pa))
		      && (START_PA_OF_SPT(pg_desc) + PAGE_SIZE < guest_end_pa)))
			return FALSE;
		return TRUE;
	}
	return FALSE;
...
BOOL l2PT_checker(addr_t l2_base_pa_add, uint32_t l2_desc)
{
	...
	if ((bft_entry->refcnt < (MAX_30BIT - 1024))
	    && l2Pt_desc_ap(l2_base_pa_add, pg_desc))
		return TRUE;
	else
		return FALSE;
}

BOOL l2Desc_validityChecker_dispatcher(uint32_t l2_type, uint32_t l2_desc,
...
uint32_t dmmu_create_L2_pt(addr_t l2_base_pa_add)
{
	...
	uint32_t guest_start_pa = curr_vm->config->pa_for_pt_access_start;
	uint32_t guest_end_pa = curr_vm->config->pa_for_pt_access_end;
	if (!(l2_base_pa_add >= (guest_start_pa)
	      && l2_base_pa_add + PAGE_SIZE < guest_end_pa))
		return ERR_MMU_OUT_OF_RANGE_PA;
...
#define CREATE_L2_DESC(x, y) (L2_BASE_MASK & x) | (L2_DESC_ATTR_MASK & y) | (0x2)
		      ...
		      addr_t page_pa_add, uint32_t attrs)
{
	...
	if (!((l2_base_pa_add >= (guest_start_pa))
	      && (l2_base_pa_add + PAGE_SIZE < guest_end_pa)))
		return ERR_MMU_OUT_OF_RANGE_PA;
	if (!((page_pa_add >= (guest_start_pa))
	      && (page_pa_add + PAGE_SIZE < guest_end_pa)))
		return ERR_MMU_OUT_OF_RANGE_PA;
