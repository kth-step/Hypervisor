...
void guests_init()
{
	...
	// this must be a loop
	temporary_create_hyper_section(flpt_va,
				       INITIAL_PT_FIXED_MAP_VA +
				       0x00000000, 0x01000000 + HAL_PHYS_START);

	temporary_create_hyper_section(flpt_va,
				       INITIAL_PT_FIXED_MAP_VA + 0x00100000,
				       0x01100000 + HAL_PHYS_START);

	temporary_create_hyper_section(flpt_va,
				       INITIAL_PT_FIXED_MAP_VA + 0x00200000,
				       0x01200000 + HAL_PHYS_START);
	...
	for (index = 0; index < 256; index++) {
		...
		ph_block_state[PA_TO_PH_BLOCK(0x01000000 + HAL_PHYS_START) +
			       index].refs = 1;
	...
	pt_create_section(guest_pt_va, 0xc0100000,
			  0x01100000 + HAL_PHYS_START, MLT_USER_RAM);

	for (index = 0; index < 256; index++) {

		ph_block_state[PA_TO_PH_BLOCK(0x01100000 + HAL_PHYS_START) +
			       index].refs = 1;
	...
	temporary_create_hyper_section(guest_pt_va, 0xc0200000,
				       0x01200000 + HAL_PHYS_START);
	...
	ph_block_state[PA_TO_PH_BLOCK(0x01200000 + HAL_PHYS_START) +
		       0].type = PAGE_INFO_TYPE_L1PT;

	ph_block_state[PA_TO_PH_BLOCK(0x01200000 + HAL_PHYS_START) +
		       1].type = PAGE_INFO_TYPE_L1PT;

	ph_block_state[PA_TO_PH_BLOCK(0x01200000 + HAL_PHYS_START) +
		       2].type = PAGE_INFO_TYPE_L1PT;
