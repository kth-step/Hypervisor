...
gtimer *timer_get(int n)
{
	...
	/* valid timer ? */
	if (n < GTIMER_FIRST || n > GTIMER_LAST)
		return 0;
...
static return_value timer_tick_handler_stub(uint32_t irq, uint32_t r1,
					    uint32_t r2)
{
...
void soc_timer_init()
{
	...
	for (i = GTIMER_FIRST; i <= GTIMER_LAST; i++) {
		...
		printf("[timer_init] GPTIMER%d at 0x%x is version %d.%d\n",
		       i, timer, (timer->tidr >> 4) & 0xF, timer->tidr & 0xF);

		// turn it off and clear interrupt
		timer->tclr |= ~GTIMER_TCLR_START_STOP_CTRL;

		timer->tisr |=
		    GTIMER_TISR_MAT_IT_FLAG_CLEAR |
		    GTIMER_TISR_OVT_IT_FLAG_CLEAR |
		    GTIMER_TISR_TCAR_IT_FLAG_CLEAR;
