...
mp_err mp_rsaep(mp_int * msg, mp_int * e, mp_int * modulus, mp_int * cipher)
{
	...
	if ((mp_cmp_z(msg) < 0) || (mp_cmp(msg, modulus) >= 0)) {
		return MP_RANGE;
	}
...
mp_err mp_rsadp(mp_int * cipher, mp_int * d, mp_int * modulus, mp_int * msg)
{
	...
	if ((mp_cmp_z(cipher) < 0) || (mp_cmp(cipher, modulus) >= 0)) {
		return MP_RANGE;
	}
...
mp_err mp_rsavp(mp_int * sig, mp_int * e, mp_int * modulus, mp_int * msg)
{
	...
	if ((mp_cmp_z(sig) < 0) || (mp_cmp(sig, modulus) >= 0)) {
		return MP_RANGE;
	}
...
mp_err mp_pkcs1v15_encodeSHA(unsigned char *msg, int mlen, char *emsg,
			     int emlen)
{
	int poffset;
...
mp_err mp_pkcs1v15_encode(char *msg, int mlen, char *emsg, int emlen,
			  rnd_f rand)
{
	int poffset;
...
mp_err mp_pkcs1v15_sign(unsigned char *hval, int mlen, mp_int * d,
			mp_int * modulus, char **out, int *olen)
{
...
mp_err mp_pkcs1v15_verify(unsigned char *hval, int mlen, mp_int * e,
			  mp_int * modulus, char **out, int *olen)
{
	int k;
	char *buf;
			   ...
			   char *out, int *olen, rnd_f rand)
{
	...
	if ((buf = malloc(k)) == NULL) {
		printf
		    ("RSA encryption failed: Out of memory when using malloc\n");
		return MP_MEM;
	}
...
mp_err mp_pkcs1v15_decrypt(char *msg, int mlen,
			   mp_int * d, mp_int * modulus, char *out, int *olen)
{
