...
mp_err mp_xgcd(mp_int * a, mp_int * b, mp_int * g, mp_int * x, mp_int * y)
{
	...
	if ((res = mp_init(&D)) != MP_OKAY)
		goto CLEANUP;
	clean[++last] = &D;
	if ((res = mp_init_copy(&xc, a)) != MP_OKAY)
		goto CLEANUP;
	clean[++last] = &xc;
	mp_abs(&xc, &xc);
	if ((res = mp_init_copy(&yc, b)) != MP_OKAY)
		goto CLEANUP;
	clean[++last] = &yc;
	mp_abs(&yc, &yc);
	...
	for (;;) {
		while (mp_iseven(&u)) {
			...
			if (mp_iseven(&A) && mp_iseven(&B)) {
				...
				if ((res = mp_add(&A, &yc, &A)) != MP_OKAY)
					goto CLEANUP;
				s_mp_div_2(&A);
				if ((res = mp_sub(&B, &xc, &B)) != MP_OKAY)
					goto CLEANUP;
				s_mp_div_2(&B);
			}
		}
		...
		while (mp_iseven(&v)) {
			...
			if (mp_iseven(&C) && mp_iseven(&D)) {
				...
				if ((res = mp_add(&C, &yc, &C)) != MP_OKAY)
					goto CLEANUP;
				s_mp_div_2(&C);
				if ((res = mp_sub(&D, &xc, &D)) != MP_OKAY)
					goto CLEANUP;
				s_mp_div_2(&D);
			}
		}
...
mp_err s_mp_grow(mp_int * mp, mp_size min)
{
	if (min > ALLOC(mp)) {
		...
		min =
		    ((min + (s_mp_defprec - 1)) / s_mp_defprec) * s_mp_defprec;
