...
int rsaEncrypt(char *g_message, char *g_e, char *g_modulus,
	       unsigned char *encrypted)
{
	...
	printf("Encrypting message, %d bytes:\n\t\"%s\"\n",
	       strlen(g_message), g_message);

	res =
	    mp_pkcs1v15_encrypt(g_message, strlen(g_message),
...
int rsaDecrypt(char *g_message, char *g_d, char *g_modulus,
	       unsigned char *decrypted)
{
	...
	res =
	    mp_pkcs1v15_decrypt(g_message, olen,
