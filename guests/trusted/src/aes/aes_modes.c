...
#pragma pack(16)
	...
	aligned_array(unsigned long, enc_gen_table, 12, 16) = NEH_ENC_GEN_DATA;
	 aligned_array(unsigned long, enc_load_table, 12, 16) =
	    NEH_ENC_LOAD_DATA;
	 aligned_array(unsigned long, enc_hybrid_table, 12, 16) =
	    NEH_ENC_HYBRID_DATA;
	 aligned_array(unsigned long, dec_gen_table, 12, 16) = NEH_DEC_GEN_DATA;
	 aligned_array(unsigned long, dec_load_table, 12, 16) =
	    NEH_DEC_LOAD_DATA;
	 aligned_array(unsigned long, dec_hybrid_table, 12, 16) =
	    NEH_DEC_HYBRID_DATA;
...
/* test the code for detecting and setting pointer alignment */
	...
	AES_RETURN aes_test_alignment_detection(unsigned int n) {	/* 4 <= n <= 16 */
		...
		for (i = 0; i < n; ++i) {
			...
			if (qh == qf)
			  ...
			  || count_neq != n - 1 ? EXIT_FAILURE : EXIT_SUCCESS);
	}
	...
	AES_RETURN aes_ecb_encrypt(const unsigned char *ibuf,
		...
		if (ctx->inf.b[1] == 0xff) {
			...
			if (!ALIGN_OFFSET(ibuf, 16) && !ALIGN_OFFSET(obuf, 16)) {
				...
				aligned_auto(uint_8t, buf,
					     BFR_BLOCKS * AES_BLOCK_SIZE, 16);
				uint_8t *ip, *op;

				while (nb) {
					int m =
					    (nb > BFR_BLOCKS ? BFR_BLOCKS : nb);

					ip = (ALIGN_OFFSET(ibuf, 16) ? buf :
					      ibuf);
					op = (ALIGN_OFFSET(obuf, 16) ? buf :
					      obuf);
		...
		if (ctx->inf.b[1] == 0xff) {
			...
			if (!ALIGN_OFFSET(ibuf, 16) && !ALIGN_OFFSET(obuf, 16)
				...
				while (nb) {
					...
					via_cbc_op7(ksp, cwd, ip, op, m, ivp,
						    ivp);
	...
	AES_RETURN aes_cfb_encrypt(const unsigned char *ibuf,
				   unsigned char *obuf, int len,
				   unsigned char *iv, aes_encrypt_ctx ctx[1]) {
		...
		if ((nb = (len - cnt) >> 4) != 0) {	/* process whole blocks */
			...
			if (ctx->inf.b[1] == 0xff) {
				...
				if (!ALIGN_OFFSET(ibuf, 16)
				    && !ALIGN_OFFSET(obuf, 16)) {
					via_cfb_op7(ksp, cwd, ibuf, obuf, nb,
						    ivp, ivp);
					...
					while (nb) {
						m = (nb >
						     BFR_BLOCKS ? BFR_BLOCKS :
						     nb), nb -= m;

						ip = (ALIGN_OFFSET(ibuf, 16) ?
						      buf : ibuf);
						op = (ALIGN_OFFSET(obuf, 16) ?
						      buf : obuf);
...
#else
#ifdef FAST_BUFFER_OPERATIONS
			if (!ALIGN_OFFSET(ibuf, 4) && !ALIGN_OFFSET(obuf, 4)
			    && !ALIGN_OFFSET(iv, 4))
	...
	AES_RETURN aes_cfb_decrypt(const unsigned char *ibuf,
		...
		if ((nb = (len - cnt) >> 4) != 0) {	/* process whole blocks */
			...
			if (ctx->inf.b[1] == 0xff) {
				...
				if (!ALIGN_OFFSET(ibuf, 16)
				    && !ALIGN_OFFSET(obuf, 16)) {
					via_cfb_op6(ksp, cwd, ibuf, obuf, nb,
						    ivp);
					ibuf += nb * AES_BLOCK_SIZE;
					obuf += nb * AES_BLOCK_SIZE;
					cnt += nb * AES_BLOCK_SIZE;
			...
			if (!ALIGN_OFFSET(ibuf, 4) && !ALIGN_OFFSET(obuf, 4)
				...
				while (cnt + AES_BLOCK_SIZE <= len) {
					...
					if (aes_encrypt(iv, iv, ctx) !=
					    EXIT_SUCCESS)
						return EXIT_FAILURE;
					t = lp32(ibuf)[0], lp32(obuf)[0] =
					    t ^ lp32(iv)[0], lp32(iv)[0] = t;
					t = lp32(ibuf)[1], lp32(obuf)[1] =
					    t ^ lp32(iv)[1], lp32(iv)[1] = t;
					t = lp32(ibuf)[2], lp32(obuf)[2] =
					    t ^ lp32(iv)[2], lp32(iv)[2] = t;
					t = lp32(ibuf)[3], lp32(obuf)[3] =
					    t ^ lp32(iv)[3], lp32(iv)[3] = t;
					ibuf += AES_BLOCK_SIZE;
					obuf += AES_BLOCK_SIZE;
					cnt += AES_BLOCK_SIZE;
			} else
	...
	AES_RETURN aes_ofb_crypt(const unsigned char *ibuf, unsigned char *obuf,
		...
		if ((nb = (len - cnt) >> 4) != 0) {	/* process whole blocks */
			...
			if (ctx->inf.b[1] == 0xff) {
				...
				if (!ALIGN_OFFSET(ibuf, 16)
				    && !ALIGN_OFFSET(obuf, 16)) {
					via_ofb_op6(ksp, cwd, ibuf, obuf, nb,
						    ivp);
					ibuf += nb * AES_BLOCK_SIZE;
					obuf += nb * AES_BLOCK_SIZE;
					cnt += nb * AES_BLOCK_SIZE;
...
#define BFR_LENGTH  (BFR_BLOCKS * AES_BLOCK_SIZE)
	...
	AES_RETURN aes_ctr_crypt(const unsigned char *ibuf, unsigned char *obuf,
				 int len, unsigned char *cbuf, cbuf_inc ctr_inc,
				 aes_encrypt_ctx ctx[1]) {
		unsigned char *ip;
		...
		if (b_pos) {
			...
			if (aes_ecb_encrypt(buf, buf, AES_BLOCK_SIZE, ctx) !=
			    EXIT_SUCCESS)
				return EXIT_FAILURE;
		...
		while (len) {
			blen = (len > BFR_LENGTH ? BFR_LENGTH : len), len -=
			    blen;
			...
			if (!ALIGN_OFFSET(ibuf, 4) && !ALIGN_OFFSET(obuf, 4)
				...
				while (i + AES_BLOCK_SIZE <= blen) {
					lp32(obuf)[0] =
					    lp32(ibuf)[0] ^ lp32(ip)[0];
					lp32(obuf)[1] =
					    lp32(ibuf)[1] ^ lp32(ip)[1];
					lp32(obuf)[2] =
					    lp32(ibuf)[2] ^ lp32(ip)[2];
					lp32(obuf)[3] =
					    lp32(ibuf)[3] ^ lp32(ip)[3];
					i += AES_BLOCK_SIZE;
					ip += AES_BLOCK_SIZE;
					ibuf += AES_BLOCK_SIZE;
					obuf += AES_BLOCK_SIZE;
