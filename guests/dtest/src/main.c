...
*/
	...
	expect(++t_id,
	       "Failing to create a L2 where guest tries to use a referenced data page",
...
*/
	...
	expect(++t_id, "Successful unmapping entry in the current L1",
	       SUCCESS, res);
	...
	res =
	    ISSUE_DMMU_HYPERCALL(CMD_MAP_L1_PT, va, va2pa(va + 0x10000), 0x21);

	expect(++t_id, "Mapping a L2 table page in  the the active L1",
	       SUCCESS, res);
	...
	res =
	    ISSUE_DMMU_HYPERCALL_(CMD_MAP_L2_ENTRY, va2pa(va + 0x10000), 0xc2,
				  va2pa(va + 0x20000), 0x32);

	expect(++t_id, "Successful mapping an entry in the first L2",
	       SUCCESS, res);
	...
	res =
	    ISSUE_DMMU_HYPERCALL(CMD_UNMAP_L2_ENTRY, va2pa(va + 0x10000),
				 0xc2, 0);
	...
	expect(++t_id, "Successful creating a new L2 (This should fail)",
	       SUCCESS, res);
	...
	res =
	    ISSUE_DMMU_HYPERCALL(CMD_MAP_L1_PT, 0xC0400000,
				 va2pa(va + 0x20000), 0x21);

	expect(++t_id, "Mapping the second L2 table page in  the active L1",
	       SUCCESS, res);
...
*/
	...
	//--
	res =
	    ISSUE_DMMU_HYPERCALL(CMD_UNMAP_L1_PT_ENTRY, (va_base + 0x100000),
				 0, 0);
	...
	expect(++t_id,
	       "Mapping the first L2 table page in  the the active L1",
	       SUCCESS, res);

	res = ISSUE_DMMU_HYPERCALL(CMD_MAP_L1_PT, va_2, va2pa(l2_va_2), 0x21);

	expect(++t_id,
	       "Mapping the second L2 table page in  the the active L1",
	       SUCCESS, res);

	res =
	    ISSUE_DMMU_HYPERCALL(CMD_MAP_L1_PT, (va_base + 0x100000),
				 va2pa(l2_va_3), 0x21);
	...
	res =
	    ISSUE_DMMU_HYPERCALL_(CMD_MAP_L2_ENTRY, va2pa(l2_va_1), 0x00,
				  shared_page, 0x32);

	res =
	    ISSUE_DMMU_HYPERCALL_(CMD_MAP_L2_ENTRY, va2pa(l2_va_1), 0x01,
				  shared_page, 0x3a);
	...
	expect(++t_id, "Successful mapping entries in the first L2",
	       SUCCESS, res);

	res =
	    ISSUE_DMMU_HYPERCALL_(CMD_MAP_L2_ENTRY, va2pa(l2_va_2), 0x00,
				  shared_page, 0x32);

	//res = ISSUE_DMMU_HYPERCALL_(CMD_MAP_L2_ENTRY, va2pa(l2_va_2), 0x1, shared_page, 0x32);
	expect(++t_id, "Successful mapping entries in the second L2",
	       SUCCESS, res);
