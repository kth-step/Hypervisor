...
void test_map_l1_section()
{
	...
	attrs = 0x0;
	res = ISSUE_DMMU_HYPERCALL(CMD_MAP_L1_SECTION, va, pa, attrs);
	expect(++t_id, "Unsupported access permission", ERR_MMU_NOT_CACHEABLE,
	       res);
	...
	attrs = 0x8;
	res = ISSUE_DMMU_HYPERCALL(CMD_MAP_L1_SECTION, va, pa, attrs);
	expect(++t_id, "Unsupported access permission",
	       ERR_MMU_AP_UNSUPPORTED, res);
...
void test_l2_create()
{
	...
	expect(++t_id,
	       "Using a physical address outside the guest allowed range",
	       ERR_MMU_OUT_OF_RANGE_PA, res);
	...
	pa = va2pa(va_base + 0x204100);	//0x81100100, this address is not 4KB aligned
	res = ISSUE_DMMU_HYPERCALL(CMD_CREATE_L2_PT, pa, 0, 0);
	expect(++t_id, "Using a physical address which is not 4KB aligned",
	       ERR_MMU_BASE_ADDRESS_IS_NOT_ALIGNED, res);
	...
	attrs = 0xc2e;
	res = ISSUE_DMMU_HYPERCALL(CMD_MAP_L1_SECTION, va, pa, attrs);
	expect(++t_id, "Remapping the writable page", SUCCESS, res);
	...
	res = ISSUE_DMMU_HYPERCALL(CMD_UNMAP_L1_PT_ENTRY, va, 0, 0);
	expect(++t_id,
	       "Unmapped the section which point to the area where we want to create an L2",
	       SUCCESS, res);

	res = ISSUE_DMMU_HYPERCALL(CMD_CREATE_L2_PT, pa, 0, 0);
	expect(++t_id, "Creation of the new L2", SUCCESS, res);

	//*******************************************************//
	uint32_t pga = va2pa(va_base + 0x330000);	// data page address (notice that must is in the cacheable region)
	uint32_t idx = 0xc2;
	res = ISSUE_DMMU_HYPERCALL_(CMD_MAP_L2_ENTRY, pa, idx, pga, 0x32 | 0x8);
	expect(++t_id,
	       "Creation of a mapping from the new L2 to a data block",
	       SUCCESS, res);
	...
	expect(++t_id,
	       "Failing to create a L2 on the block that is pointed (and writable) by the previous allocated L2",
	       ERR_MMU_REFERENCED, res);

	// #5: Guest can not create a new L2 with an unsupported descriptor type (0x3)
	// First we must free the previously create L2 and remap the section
	va = (va_base | (uint32_t) 0x320000);
	pa = va2pa(va);
	res = ISSUE_DMMU_HYPERCALL(CMD_FREE_L2, pa, 0, 0);
	expect(++t_id,
	       "We free the memory hosting the previously allocated L2Removed the L2",
	       SUCCESS, res);
	...
	// now we can create an L2 with an unsupported AP
	...
	res = ISSUE_DMMU_HYPERCALL(CMD_CREATE_L2_PT, pa, 0, 0);
	expect(++t_id,
	       "Failing to create a L2 with an unsupported descriptor type (0x3)",
	       ERR_MMU_L2_UNSUPPORTED_DESC_TYPE, res);
	...
	res = ISSUE_DMMU_HYPERCALL(CMD_CREATE_L2_PT, pa, 0, 0);
	expect(++t_id,
	       "Failing to create a L2, where L2 base is in writable section",
	...
	res = ISSUE_DMMU_HYPERCALL(CMD_CREATE_L2_PT, pa, 0, 0);
	expect(++t_id,
	       "create a new L2 with an entry which point to another page table with a read-only  access permission",
	       SUCCESS, res);
	ISSUE_DMMU_HYPERCALL(CMD_FREE_L2, pa, 0, 0);
	...
	pa = va2pa(va);
	ISSUE_DMMU_HYPERCALL(CMD_MAP_L1_SECTION, va, pa, attrs);
	desc = ((pstart & 0xff000000) | (1 << 23) | 0x32 | 0x8);
	for (j = 0; j < 1024; j++)
	...
	expect(t_id,
	       ...
	       ERR_MMU_PH_BLOCK_NOT_WRITABLE, res);
	ISSUE_DMMU_HYPERCALL(CMD_FREE_L2, pa, 0, 0);
}
...
void test_l1_pt_map()
{
	...
	expect(++t_id,
	       "Map l1 entry in a virtual address which is reserved for hypervisor use",
	       ERR_MMU_RESERVED_VA, res);
	...
	res = ISSUE_DMMU_HYPERCALL(CMD_MAP_L1_PT, va, pa, attrs);
	expect(++t_id,
	       "Mapping a physical address is outside the guest allowed range",
	       ERR_MMU_OUT_OF_RANGE_PA, res);
	...
	expect(++t_id, "Mapping an entry of the L1 two times",
	       ERR_MMU_PT_NOT_UNMAPPED, res);
...
void test_l2_map_entry()
{
	...
	expect(++t_id,
	       "Mapping an entry of an invalid L2, in physical address outside the allowed range",
	       ERR_MMU_OUT_OF_RANGE_PA, res);
	...
	res = ISSUE_DMMU_HYPERCALL_(CMD_MAP_L2_ENTRY, pa, idx, pga, attrs);
	expect(++t_id,
	       "Mapping a physical address outside the guest allowed range",
	       ERR_MMU_OUT_OF_RANGE_PA, res);
	...
	res = ISSUE_DMMU_HYPERCALL_(CMD_MAP_L2_ENTRY, pa, idx, pga, attrs);
	expect(++t_id,
	       "Mapping a valid physical address into one of a valid L2",
	       SUCCESS, res);

	// #3: this test should fail, because the entry has already been mapped.
	res = ISSUE_DMMU_HYPERCALL_(CMD_MAP_L2_ENTRY, pa, idx, pga, attrs);
	expect(++t_id, "Mapping an entry two times", ERR_MMU_PT_NOT_UNMAPPED,
	       res);
	...
	expect(++t_id, "Mapping an entry of a data page!!!!",
	       ERR_MMU_IS_NOT_L2_PT, res);
...
void test_l2_unmap_pt()
{
	...
	for (idx = 0; idx < 1024; idx++) {
		res =
		    ISSUE_DMMU_HYPERCALL_(CMD_MAP_L2_ENTRY, pa, idx, pga,
					  attrs);
	...
	expect(t_id++,
	       "The small pages mapped by the new L2 points all to the same physical address",
	       666, val);
	...
	expect(t_id++, "Successfoul alias of the va address using a section",
	       SUCCESS, res);

	val = (*((uint32_t *) (va_section)));
	expect(t_id++,
	       "The small pages mapped by the new L2 points to the right physical address",
	       666, val);

	res = ISSUE_DMMU_HYPERCALL(CMD_UNMAP_L1_PT_ENTRY, va_section, 0, 0);
	expect(t_id++, "Successfoul unmap of the section aliasing the va",
	       SUCCESS, res);
	// finished

	res = ISSUE_DMMU_HYPERCALL(CMD_FREE_L2, pa, 0, 0);
	expect(t_id, "Unsuccessful freeing a referenced L2",
	       ERR_MMU_REFERENCE_L2, res);

	// #1: this test should succeed
	res = ISSUE_DMMU_HYPERCALL(CMD_UNMAP_L1_PT_ENTRY, va, 0, 0);
	expect(t_id, "Successful unmap the L1 entry pointing to the new L2",
	       SUCCESS, res);
	...
	res = ISSUE_DMMU_HYPERCALL(CMD_FREE_L2, pa, 0, 0);
	expect(t_id,
	       "Freeing L2 should faile because the base address is out of the allowed range",
	       ERR_MMU_OUT_OF_RANGE_PA, res);
	...
	res = ISSUE_DMMU_HYPERCALL(CMD_FREE_L2, pa, 0, 0);
	expect(t_id,
	       "Freeing L2 should faile because the base address is not pointing to an L2",
	       ERR_MMU_IS_NOT_L2_PT, res);
}
...
void test_l1_create()
{
	...
	res = ISSUE_DMMU_HYPERCALL(CMD_CREATE_L1_PT, pa, 0, 0);
	expect(++t_id,
	       "Failing to create a L1 on referenced part of the memory",
	       ERR_MMU_REFERENCED, res);

	// #4: Test that we can not create an L1 in a region that is not always cacheable
	pa = va2pa(va_base);
	res = ISSUE_DMMU_HYPERCALL(CMD_CREATE_L1_PT, pa, 0, 0);
	expect(++t_id, "Failing to create a L1 on non-cacheable memory",
	       ERR_MMU_OUT_OF_CACHEABLE_RANGE, res);
	...
	res = ISSUE_DMMU_HYPERCALL(CMD_CREATE_L1_PT, pa, 0, 0);
	expect(++t_id,
	       "Failing to create a L1 using an L2 that has not been previously validated",
	       ERR_MMU_IS_NOT_L2_PT, res);
	...
	l1[512] = (((uint32_t) pa) & 0xFFF00000) | (uint32_t) 0xC02 | (uint32_t)
	    0x8;
	memcpy((void *)va, l1, sizeof l1);
	...
	expect(++t_id,
	       ...
	       ERR_MMU_NEW_L1_NOW_WRITABLE, res);
	...
	res = ISSUE_DMMU_HYPERCALL(CMD_CREATE_L1_PT, pa, 0, 0);
	expect(++t_id,
	       "Failing to create a L1 where a non-cacheable section points to the always cacheable region",
	       ERR_MMU_NOT_CACHEABLE, res);
...
void test_l1_create_and_switch_l1()
{
	...
	*((uint32_t *) (va + 1024)) = (uint32_t) 666;
	expect(++t_id, "Successful section modification", 666,
	       *((uint32_t *) (va + 1024)));
	...
	l1[3072] =
	    (((uint32_t) va2pa(0xc0000000)) & 0xFFF00000) | (uint32_t) 0xC2E;
	l1[513] =
	    (((uint32_t) va2pa(0xc0100000)) & 0xFFF00000) | (uint32_t) 0x82C;
	memcpy((void *)va, l1, sizeof l1);

	// 5) Un-mapping the new L1 section
	res = ISSUE_DMMU_HYPERCALL(CMD_UNMAP_L1_PT_ENTRY, va, 0, 0);
	expect(++t_id,
	       "Successful unmapping an entry of l1 which points to a section where the new L1 resides in",
	       SUCCESS, res);
	...
	expect(++t_id, "Successful map of the new page", SUCCESS, res);
	printf
	    ("reading the content of the modified page under new L1, content %x \n",
	     *((uint32_t *) (va + 1024)));
...
void test_switch_mm()
{
	...
	res = ISSUE_DMMU_HYPERCALL(CMD_SWITCH_ACTIVE_L1, pa, 0, 0);
	expect(++t_id,
	       "Failing to switch to the given physical address which is out of guest allowed  memory range",
	       ERR_MMU_OUT_OF_RANGE_PA, res);
	...
	res = ISSUE_DMMU_HYPERCALL(CMD_SWITCH_ACTIVE_L1, pa, 0, 0);
	expect(++t_id,
	       "Failing to switch to the given L1 physical address which not 16KB aligned",
	       ERR_MMU_L1_BASE_IS_NOT_16KB_ALIGNED, res);
	...
	res = ISSUE_DMMU_HYPERCALL(CMD_SWITCH_ACTIVE_L1, pa, 0, 0);
	expect(++t_id,
	       "Failing to switch to the given physical address because guest is trying to switch into a non-page table page",
	       ERR_MMU_IS_NOT_L1_PT, res);
...
void test_unmap_L1_pt()
{
	...
	expect(t_id,
	       "Unsuccessful unmapping a physical address outside the guest allowed range",
	       ERR_MMU_OUT_OF_RANGE_PA, res);
	...
	res = ISSUE_DMMU_HYPERCALL(CMD_FREE_L1, pa, 0, 0);
	expect(t_id,
	       "Unsuccessful unmapping a physical address which is not 16KB aligned",
	       ERR_MMU_L1_BASE_IS_NOT_16KB_ALIGNED, res);

	// #2: this test should fail because base address is pointing to the active L1 page table
	pa = va2pa(va_base + 0x200000);
	res = ISSUE_DMMU_HYPERCALL(CMD_FREE_L1, pa, 0, 0);
	expect(t_id, "Unsuccessful unmapping the active L1 page table",
	       ERR_MMU_FREE_ACTIVE_L1, res);

	// #3: this test should fail because base address is not pointing to a L1 page table
	pa = va2pa(va_base + 0x1C0000);
	res = ISSUE_DMMU_HYPERCALL(CMD_FREE_L1, pa, 0, 0);
	expect(t_id,
	       "Unsuccessful unmapping a physical address which is not pointing to a L1 page table",
	       ERR_MMU_IS_NOT_L1_PT, res);
	...
	res = ISSUE_DMMU_HYPERCALL(CMD_FREE_L1, pa, 0, 0);
	expect(t_id, "Unmapping the guest initial master page table", SUCCESS,
	       res);
