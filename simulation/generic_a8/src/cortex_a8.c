		    ...
		    icmProcessorP * cpu_, icmBusP * bus_)
{
	...
	const char *armModel = icmGetVlnvString(0,	// path (0 if from the product directory)
	...
	const char *armSemihost = icmGetVlnvString(0,
						   "arm.ovpworld.org",
						   "semihosting",
						   "armNewlib",
						   "1.0",
	...
	if (debug)
		procAttrs |= ICM_ATTR_DEBUG;
	icmProcessorP processor = icmNewProcessor("Cortex-A8",	// name
						  "arm",	// type
						  0,	// cpuId
						  cpu_flags,	// flags
						  32,	// address bits
...
//        icmDebugThisProcessor(processor);
	...
	const char *cpuHelper = icmGetVlnvString(0,
						 "imperas.com", "intercept",
						 "armCpuHelper", "1.0",
						 "model");

	icmAddInterceptObject(processor,	// Processor handle
	...
	const char *AIC_Model = icmGetVlnvString(0,	// path (0 if from the product directory)
	...
	icmPseP AIC = icmNewPSE("AIC",	// name
				AIC_Model,	// model
	...
	const char *wctModel = icmGetVlnvString(0,	// path (0 if from the product directory)
						"atmel.ovpworld.org",	// vendor
						"peripheral",	// library
						"WatchdogTimer",	// name
						"1.0",	// version
	...
	icmPseP watchDogTimer = icmNewPSE("WD",	// name
					  wctModel,	// model
	...
	const char *psModel = icmGetVlnvString(0,	// path (0 if from the product directory)
					       "atmel.ovpworld.org",	// vendor
					       "peripheral",	// library
					       "PowerSaving",	// name
					       "1.0",	// version
	...
	icmPseP powerSaving = icmNewPSE("PS",	// name
					psModel,	// model
					psAttrList,	// attrlist
					0,	// semihost file
					0	// semihost symbol
	...
	const char *pIOModel = icmGetVlnvString(0,	// path (0 if from the product directory)
						"atmel.ovpworld.org",	// vendor
						"peripheral",	// library
	...
	icmPseP pio = icmNewPSE("PIO",	// name
				pIOModel,	// model
	...
	const char *tcModel = icmGetVlnvString(0,	// path (0 if from the product directory)
					       "atmel.ovpworld.org",	// vendor
					       "peripheral",	// library
					       "TimerCounter",	// name
					       "1.0",	// version
	...
	icmPseP timerCounter = icmNewPSE("TC",	// name
					 tcModel,	// model
					 tcAttrList,	// attrlist
					 0,	// semihost file
					 0	// semihost symbol
	...
	const char *sfModel = icmGetVlnvString(0,	// path (0 if from the product directory)
					       "atmel.ovpworld.org",	// vendor
					       "peripheral",	// library
	...
	icmPseP specialFunction = icmNewPSE("SF",	// name
					    sfModel,	// model
					    sfAttrList,	// attrlist
					    0,	// semihost file
					    0	// semihost symbol
	...
	icmConnectPSEBus(specialFunction, bus, "bp1", 0, 0xfff00000,
			 0xfff03fff);
	...
	icmMemoryP ram =
	    icmNewMemory("ram0", ICM_PRIV_RWX, 1024 * 1024 * 16 - 1);
	icmConnectMemoryToBus(bus, "sp1", ram, 0x0);

	/* my additional 32MB of SRAM at 0x8000_0000 */
	icmMemoryP ram2 =
	    icmNewMemory("ram2", ICM_PRIV_RWX, 1024 * 1024 * 32 - 1);
	...
	icmPseP icmPseP_dmac =
	    icmNewPSE("dmac", "obj/dmacModel.pse", NULL, NULL, NULL);

	// connect the DMAC slave port on the bus and define the address range it occupies
	icmConnectPSEBus(icmPseP_dmac, bus, "DMACSP", False, 0xfff10000,
			 0xfff1013f);
	icmConnectPSEBus(icmPseP_dmac, bus, "MREAD", True, 0x00000000, 0xffffffff);	//DMA_TODO: limit range here?
	icmConnectPSEBus(icmPseP_dmac, bus, "MWRITE", True, 0x00000000,
			 0xffffffff);
	...
	const char *USART_PSE = icmGetVlnvString(0,	// path (0 if from the product directory)
						 "atmel.ovpworld.org",	// vendor
						 "peripheral",	// library
						 "UsartInterface",	// name
						 "1.0",	// version
	...
	const char *USART_Model = icmGetVlnvString(0,	// path (0 if from the product directory)
						   "atmel.ovpworld.org",	// vendor
						   "peripheral",	// library
						   "UsartInterface",	// name
						   "1.0",	// version
	...
	icmPseP USART = icmNewPSE("USART0",	// name
				  USART_PSE,	// model
