...
int getNumber(icmProcessorP processor, char *str, int *result)
{
	...
	if (getNumberFromPointer(processor, str, result))
		return 1;

	if (getNumberFromString(str, result))
		return 1;

	if (getNumberFromRegister(processor, str, result))
		return 1;

	if (getAddressFromSymbol(str, result))
		return 1;

	if (getNumberFromVariable(str, result))
		return 1;
...
void print_dissambly(icmProcessorP processor, Uns32 adr, char *marker)
{
	...
	printf("%s 0x%08x %s %s",
	       isBreakpoint(adr) ? "*b*" : "   ",
	       adr, marker, icmDisassemble(processor, adr));
...
void print_memory_line(int adr, unsigned int *val, unsigned int *flags,
		       int count)
{
...
void doListProgram(icmProcessorP processor)
{
	...
	if (params_has_flag('f')) {
		...
		if (!getClosestSymbolBefore(pos, &adr1, 0))
			adr1 = pos;

		else if ((unsigned int)(pos - adr1) > 256)
			adr1 = pos - 256;

		if (!getClosestSymbolAfter(pos, &adr2, 0))
			adr2 = pos + 12;
...
void doWriteMemory(icmProcessorP processor)
{
	...
	for (;;) {
		...
		if (!getNumberFromString(buffer, &data)) {
			...
			if (!icmDebugWriteProcessorMemory
...
void doSetWatchpoint(icmProcessorP processor)
{
	...
	if (params_has_flag('p'))
		physical = 1;

	if (params_has_flag('w'))
		write_access = 1;
...
int executeCommand(icmProcessorP processor, char *buffer)
{
	...
	if (tmp) {
		...
		*tmp++ = '\0';
		...
		if (executeCommand(processor, buffer))
			return True;
